...
function CreateSSNewList ( ) {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            ...
            try {
                const userResponse = await axios.get(`https://back-it.onrender.com/api/user/${userId}`);
    ...
    const handleCreateSpreadsheet = async (Title) => {
        ...
        try {
            ...
            const response = await axios.post('https://back-it.onrender.com/api/spreadsheet', {
                name: Title,
                owner: userId
            });
    ...
    return (
        <div className="">
            <button
                ...
                className="min-w-full relative text-left font-medium bg-blue-700 text-white rounded py-1 px-3 hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
                <FaPlus className="mb-1 inline mr-1" />
                New
            </button>
            <Modal
                isOpen={isBoxOpen}
    ...
    return (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50" onClick={handleOverlayClick}>
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm mx-4 sm:mx-6 lg:max-w-md lg:mx-8">
                <h2 className="text-black text-lg font-semibold mb-4">Create a New Spreadsheet</h2>
...
export default CreateSSNewList;
