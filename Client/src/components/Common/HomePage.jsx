...
const HomePage = () => {
    ...
    const toggleBoolean = ()=>{
        ...
        e.preventDefault();
        try {
            const response = await axios.post('https://back-it.onrender.com/api/login', { username, password });
        ...
        e.preventDefault();
            try {
            const response = await axios.post('https://back-it.onrender.com/api/register', { username, password });
            if (response.status === 200) {
